...
useEffect(() => {
    console.log("fetch running", id);
    if (!id) return;

    async function loadReport() {
      try {
        const data = await fetchWithAuth(`${API_PURCHASE}/${id}`);
    ...
    <div className="flex flex-1 flex-col">
      ...
      <div className="@container/main flex flex-1 flex-col gap-2">
        <div className="w-full flex flex-col px-4 lg:px-6 items-start pt-2 md:pt-6">
          ...
          <div className="w-full flex flex-col lg:flex-row items-start md:items-center justify-between gap-4 py-2 md:gap-6 md:py-4">
            ...
            <div className="flex gap-2">
              <Button variant={"outline"} size={"lg"} className="text-xs lg:text-base"><IconEdit /> Edit Sumber Dana</Button>
              <Button variant={"default"} size={"lg"} className="text-xs lg:text-base"><IconPlus /> Tambah Laporan</Button>
            </div>
          </div>
          <div className="flex flex-col lg:flex-row w-full items-start md:items-center gap-2 justify-between">
            <div className="flex flex-col items-start">
              <span className="text-sm items-start">Terakhir Dibuat: {new Date(report?.created_at).toLocaleString('en-GB')}</span>
              <span className="text-sm items-start">Terakhir Diperbarui: {new Date(report.updated_at).toLocaleString('en-GB')}</span>
            </div>
        ...
        <div className="flex flex-col gap-4 py-2 md:gap-6 md:py-6">
          <DataTable data={report?.activities || []} />
        </div>
      </div>
    </div>
  );
