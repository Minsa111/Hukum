...
import { API_PURCHASE, API_PUBLIC } from "@/api/api";
...
export default function Page(isSuperAdmin: boolean) {
  ...
  async function loadReport() {
    try {
      const data = await fetchWithAuth(`${API_PURCHASE}${isSuperAdmin ? API_PUBLIC:""}/${purchase_report_id}`);
    ...
    <div className="flex flex-1 flex-col">
      ...
      <div className="@container/main flex flex-1 flex-col gap-2">
        <div className="w-full flex flex-col px-4 lg:px-6 items-start pt-2 md:pt-6">
          ...
          <div className="flex flex-col lg:flex-row w-full items-start md:items-center gap-2 justify-between">
            <div className="flex flex-col items-start">
              {isSuperAdmin ? <span className="text-xs sm:text-sm items-start">Sekolah: {report.school_name}</span> : ""}
        ...
        <div className="flex flex-col gap-4 py-2 md:gap-6 md:py-6">
          <DataTable activities = {report.activities} onDataChange={loadReport} isPublic={true}/>
        </div>
      </div>
    </div>
  );
