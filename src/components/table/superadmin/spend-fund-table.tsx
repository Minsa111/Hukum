...
export function DataTable({report, nisn, selectedYear, isSuperAdmin}:{report:any[], nisn:string, selectedYear:string, isSuperAdmin:boolean}) {
  ...
  const columns: ColumnDef<z.infer<typeof reportSchema>>[] = [
      ...
      cell: ({ row }) => {
        return (
          <div className="text-left truncate w-64 lg:w-sm px-2 lg:px-4">
            <Link to={`/superadmin/pembelanjaan/${row.original.id}`}>
              {row.original.title}
            </Link>
          </div>
        )
    ...
    {
      accessorKey: "Total Dana Terealisasi",
      ...
      cell: ({ row }) => {
        ...
        return(
          <div className="w-32 text-left px-2 lg:px-4">
            Rp. {totalRealization ? 
              totalRealization.toLocaleString("id-ID")
              :
            "0"}
          </div>
      )},
    ...
    },
    {
      ...
      header: "Laporan Terakhir Diperbarui",
      cell: ({ row }) => {
    ...
    {
      accessorKey: "Laporan Dibuat Pada",
      header: "Laporan Dibuat Pada",
      cell: ({ row }) => {
    ...
    {
      accessorKey: "Terakhir Aktivitas Diperbarui",
      header: "Terakhir Aktivitas Diperbarui",
      cell: ({ row }) => {
        let lastUpdated: Date | null = null;

        row.original.activities?.forEach((act: any) => {
          const actDate = new Date(act.updated_at);
          lastUpdated = lastUpdated || actDate;
          if (!lastUpdated || actDate > lastUpdated) {
            lastUpdated = actDate;
          }
        });
        return (
